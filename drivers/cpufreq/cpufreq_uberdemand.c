 ...
 * rate is 10 ms because the user needs immediate reaction from uberdemand
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {	
		if (policy->cur < policy->max) {
			if (policy->cur < 422400) dbs_freq_increase(policy, 729600);
			else if (policy->cur < 883200) dbs_freq_increase(policy, 960000);
			else {
...
static int dbs_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (policy->cur < src_freq) {
			/* cancel the next uberdemand sample */
			...
			/* reschedule the next uberdemand sample */
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dbs_wq = alloc_workqueue("uberdemand_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create uberdemand_dbs_wq workqueue\n");
		return -EFAULT;
	}
